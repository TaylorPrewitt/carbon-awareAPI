...
def msai_endpoint():
    ...
    current_region = request.args.get("data_center_name", 'East US') # Given as display name. Convert to region name
    ...
    SKU_table = get_SKU_table()
    sensitive_check_box = request.args.get('sensitive', 'off') # Change this to request.form.get with HTML page added on
    desired_GPU = request.args.get("gpu_type", "nada") # Change this to request.form.get with HTML page added on
    ...
    if deltaminutes < 30: deltaminutes = MINUTES_IN_HOUR 
    ...
    for az_region, time_moer in all_regions_forecasts.items():
        ...
        """
        current_moer_sum = -1 # If we see a -1, it just means there wasn't any forecast data available. 
        ...
        for moer_val, point_time in zip(moer_vals, point_times):
            ...
            if end_time > datetime.datetime.fromisoformat(point_times[-1]) or point_time > end_time:
                break
            if point_time < current_time: continue
            ...
            #    current_moer_vals.append(moer)
        all_region_moer_comparison[az_region]['moer_avg'] = current_moer_sum/(deltaminutes/5)

    current_region_moer_sum = all_region_moer_comparison[current_region]['moer_avg']
    for region in all_region_moer_comparison.keys():
        if region == current_region or region == "current_region": continue # A key dictating the current region selected is in there. Also skip doing a comparison for the current region. 0% increase/decrease
        #print(all_region_moer_comparison[region])
        #continue
        region_sum = all_region_moer_comparison[region]['moer_avg']
        perc_diff = ((region_sum - current_region_moer_sum) / current_region_moer_sum) * 100
...
def amulet_endpoint():
    ...
    deltaminutes = int(request.args.get("window_size_minutes", 60)) #default to 60 minutes for window size
    # NameError
    ...
    for region_name in NAME_TO_DISPLAY.keys(): # keys are all the AZ region names. 
        avg_moer_value = get_avg_moer(region_name = region_name, starting_time=current_time, deltaminutes=deltaminutes)
...
def get_current_min_region(filtered_regions_list,current_region):
    ...
    for entry in region_average_dict:
        if float(region_average_dict[entry]["average_moer"]) == moer_min:
            min_region_name.append(region_history_dict[0]["Azure Region"][entry])
    ...
    geo_shift_perc_diff = 100*(float(current_region_data['moer']) - float(moer_min))/float(current_region_data['moer'])
...
def update_data_table():
    ...
    if not os.path.isdir("./local_files"): #If this directory doesn't exist, the json file containing the history doesn't as well.
        ...
        if len(data) >= HISTORY_LIMIT: # only hold up to HISTORY_LIMIT number of previous calls
            data = data[-(HISTORY_LIMIT - 1):] # Will only keep 1 less than the limit, then add the latest entry.
