...
def get_SKU_table():
    csv_url = os.path.join(
        current_app.config['ROOT_DIR'], "app/static", "Region-SKU-Tags.csv")
...
def Law_filter(SKU_data_table, sensitive_check_box, Az_region):
    ...
    try:
        if sensitive_check_box == 'on':
            ...
            SKU_data_table_filtered = SKU_data_table[SKU_data_table['R_Tag_lock'].isin(SKU_data_table[SKU_data_table['region'] == Az_region]['R_Tag_lock'])].reset_index(
            )
        else:
            SKU_data_table_filtered = SKU_data_table.reset_index()
    except KeyError:   # if name was provided as an name not displayName
        if sensitive_check_box == 'on':
            SKU_data_table_filtered = SKU_data_table[SKU_data_table['R_Tag_lock'].isin(SKU_data_table[SKU_data_table['region'] == NAME_TO_DISPLAY[Az_region]]['R_Tag_lock'])].reset_index(
            )
...
def Gpu_filter(SKU_data_table_filtered, desired_GPU):
    ...
    if desired_GPU == 'nada':
        ...
        return [region for region in SKU_data_table_filtered['region']]
    else:
        return [region for region, sku in SKU_data_table_filtered[['region', 'sku']].itertuples(index=False) if desired_GPU in sku]
...
def find_forecast_window_min(some_dict, start_index, end_index, window_size):
    ...
    print(
        f"Here is a list of the moer vals in that window for confirmation:\n {min_moer_vals}")
...
def get_current_min_region(filtered_regions_list, current_region):
    ...
    position_numbers = [
        ...
        entry for entry in region_history_dict[0]["Azure Region"] if region_history_dict[0]["Azure Region"][entry] in filtered_regions_list]
