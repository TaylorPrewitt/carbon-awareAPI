...
def DataPrep(sku, start, end):
    ...
    return gpu_type, starttime, endtime
...
def counterfactual_emissions(available_regions, starttime, endtime, token, row):
    '''
    Parameters
    ----------
    ...
    for region in available_regions:
        ...
        shift_location_data = getloc_helper(region, token)
    
        # get emissions data for the run and convert to dict
        shift_emissions_data_string  = gather_watttime(shift_location_data["abbrev"], starttime, endtime, token, row).text
        ...
        counterfactual.append({'region':region, 'mean_run_emission':mean_run_emission})
...
def counterfactual_evaluation(data_set, duration):
    ...
    for row in duration:
        ...
        # grab run parameters
        gpu_type, starttime, endtime = DataPrep(data_set["VMSize"][row], data_set["StartTime"][row], data_set["FinishTime"][row])
        ...
        # restrict to regions with the hardware available
        available_regions = Gpu_filter(GPU_data_table_filtered, gpu_type)   
        ...
        # get average marginal emissions over the window of the run in used region
        shifting_counterfactual = counterfactual_emissions(available_regions, starttime, endtime, token, row)
        
        # finding the greenest counterfactual 
        green_region_info = min_counterfactual(shifting_counterfactual)
        #print(f"the green region was {green_region_info['region']} but the run was exectuted in {cluster_region}")
        #print(f"the green average emission was {green_region_info['mean_run_emission']} but the run average emissions was {mean_run_emission}")
        
        # calculating the percent difference for the green region compared to used region
        carbon_delta_percent = MOER_PercentDifference(mean_run_emission, green_region_info['mean_run_emission'])
        #print(f"the counterfactrual percent difference for {cluster_region} compared to {green_region_info['region']} is {carbon_delta_percent}%")
        
     
        data_set_counterfactual.append({'current_region':cluster_region, 'green_region':green_region_info['region'], 'carbon_delta_percent':carbon_delta_percent, 'index_value':row})
       

    return data_set_counterfactual


# EXAMPLE.  submits first 10 rows for counterfactual evaluation
duration = range(0,10)
print("evaluation in-progress....")
data_set_eval = counterfactual_evaluation(data_set = data_set, duration = duration)
data_set_eval_df = pd.DataFrame(data_set_eval)
data_set_eval_df.to_csv('data_set_eval.csv')
